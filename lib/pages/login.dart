...
class _LoginPageState extends State<LoginPage> {
    ...
    try {
      ...
      setState(() async {
        ...
        if (map['status'] == 'success') {
          ...
          _handleClickMe(getTranslated(context, 'loggedin-successfuly-message'));
        }
      });
      // Navigator.pop(context);
    } catch (e) {
  ...
  _loginWithFB(fbID,name,email) async{
    try {
      var response = await http.post(Utils.REGISTERWITHFACEBOOK_URL,
                                  body: {
                                      'fb_id'    : fbID,
                                      'name'    : name,
                                      'email'   : email,
                    ...
                    Positioned(
                      ...
                      child: FlatButton(
                        ...
                        child: Column(
                          ...
                          children: <Widget>[
                            Text(
                              getTranslated(context, 'can-not-remember-passord'),
                              style: TextStyle(
                                color: Colors.gold
                              ),
                            ),
